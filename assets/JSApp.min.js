...
//		this.model.bind('change', this.update)
	...
	*/
	update: function(){ // DEBUG: window.console.log('now in update')
		var i = this.model.get("state"), j
		j = (i < 4) ? this.model.resmx[i] : null
		// sync the text and radio states
		// TODO: Replace with this.$(':radio')[i].checked = true ???
		this.$el.find(":radio")[i].checked = true
		this.$el.find(':text').val(j).toggleClass('gtext', (i > 2) && (j != null))
		// add visual cues
		// this.$el.toggleClass('bg-color-blueDark', (i<3)).toggleClass('bg-color-blueLight', (i==3))
		this.$el.toggleClass('bg-color-blueDark', (i<4))
	}
});
...
//		_.bindAll(this, 'setResult')
//		this.bind('change', this.setResult)
	...
	post: function(){
		$.post("./scripts/post.php", {
			"PerId" : this.identity.PerId , "models": JSON.stringify(this) },
			/* as a test, write-back the data received at the server */
			function(data){ window.console.log(data) }
		)
	}
});
/* App container */
...
//		this.collection.bind('change:title', this.update)
	},
	tabResult: function(){
