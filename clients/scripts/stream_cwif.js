...
// Comparison function for points based ranking
...
// Re-calculate rankings based on points
const recalculateRank = data => {
  ...
  const pts = data.sort(comparePoints).map(v => v.points)
  const rnk = data.map(v => pts.indexOf(v.points)+1)
  
  data.forEach((x, i) => x.result_rank = rnk[i])
  return data
}
...
const fetchResults = params => {
  return fetch('http://'+options.url+'/results/route', { 
    ...
    recalculateRank(r.body).forEach(x => { 
...
const handleStream = data => {
  recalculateRank(JSON.parse(data)).forEach(newResult => {
