...
_.mixin({
	...
	//	www.webxpertz.net/forums/showthread.php/11030-Article-JavaScript-Sorting-a-Multidimensional-Array-—-quicksort
	//
...
App.ResultsList = Backbone.Collection.extend({
	...
	loadResults: function(){
		var query = '?WetId='+this.comp+'&GrpId='+this.cat+'&route='+this.round,
			self  = this, func = function(data){
				self.parseJSONData(data.participants)
				self.reset(data.participants)
				self.setRankPrevHeat()
				self.sortResults(data.participants)
			}
	...
	loadStarters: function(){
		var query = '?comp='+this.comp+'&type=starters',
			show  = this.settings.get('ShowCategory'),
			self  = this, func = function(data){
				// Edit the response to trim out team officials & competitors not in the currently
				// 'active' category
				data.athletes = _(data.athletes).filter(function(athlete){
					athlete.cat = parseInt(athlete.cat, 10)
					return show ? (athlete.cat === self.cObj.male) : (athlete.cat === self.cObj.female)
				})

				// Parse the remaining data and create a set of models
				self.parseJSONData(data.athletes)
				self.reset(data.athletes)
			}
	...
	update: function(){
		var	round = this.round,
			query = '?WetId='+this.comp+'&GrpId='+this.cat,
			self  = this, func = function(data){ self.sortResults(data.participants) }
	...
	parseJSONData: function(participants){
		...
		_(participants).each(function(person, i){
			...
			person.id             = parseInt(person.PerId, 10)
...
App.FullScreenMixins.ResultsListView = {
	...
	autoRefresh: function(){
		var self     = this,
			...
			interval = this.currentRound ? 10000 : 30000
...
document.addEventListener('DOMContentLoaded', function(){
	var idx  = 10,
		cmps = { "WetId" : 1, "type" : "B", "name" : "CWIF (2016)" },
		defs = { "disciplines" : ['B'], "cat_id" : [69] },
