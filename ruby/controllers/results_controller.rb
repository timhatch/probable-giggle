...
#   (In theory the relevant method in LocalDBConnection will cope with multiple boulders being updated
...
module Perseus
  class ResultsController < Perseus::ApplicationController
    ...
    # Convert the received parameters into hash symbols and call LocalDBConnection.get_result_person
    get '/person' do
      LocalDBConnection::Results.result_person(params).to_json
    end

    # Fetch __multiple__ results (i.e. for a route)
    # Convert the received parameters into hash symbols and call LocalDBConnection.get_result_route
    get '/route' do
      LocalDBConnection::Results.result_route(params).to_json
    end

    # Update a __single__ result
    # Convert the received parameters into hash symbols and call LocalDBConnection.set_result_single
    put '/person' do
